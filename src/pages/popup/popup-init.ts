...
function initSendTokenWidget(windowManager) {
  ...
  stW.destinationInput = document.getElementById(
    'send-token-widget-destination'
  );
...
class Credentials {
  ...
  retrieveCredentials(source) {
    this.retrieveCredentialsFromStorage(
      mapicredentials => {
  ...
  retrieveCredentialsFromStorage(success, failure) {
    chrome.storage.local.get('mapicredentials', mapicredentials => {
      ...
      if (
        Object.getPrototypeOf(mapicredentials) === Object.prototype &&
        !mapicredentials
      ) {
        failure();
        return;
  ...
  storeCredentials(username, password) {
    ...
    chrome.storage.local.set(
      { mapicredentials: username + '*' + password },
      function () {
        if (chrome.runtime.lastError) {
          console.log(chrome.runtime.lastError);
...
class EnterCredentialsWidget {
  ...
  show(source) {
    ...
    this.saveButton.onclick = () => {
      this.close();
      source.credentialsEnteredListener(
...
function initMAPICredentialsAction() {
  ...
  mca.cancelButton = document.getElementById(
    'request-mapi-credentials-span-cancel'
  );
  mca.usernameField = document.getElementById(
    'request-mapi-credentials-span-username'
  );
  mca.passwordField = document.getElementById(
    'request-mapi-credentials-span-password'
  );
...
function initPKCreateWidget() {
  ...
  pkCreateWidget.twelveWordsWidget = document.getElementById(
    'pk-create-span-12words-widget'
  );
  pkCreateWidget.twelveWordsText = document.getElementById(
    'pk-create-span-12words-text'
  );
...
function initReceiveWidget(windowManager) {
  ...
  receiveOp.copyAddressInput = document.getElementById(
    'copy-receive-span-address'
  );
  ...
  refreshBalanceListener(totals) {
    ...
    if (totals.tokenBalances.length > 0) {
      ...
      for (let i = 0; i < totals.tokenBalances.length; i++) {
        this.createTokenRow(
          totals.tokenBalances[i].symbol,
          totals.tokenBalances[i].balance
        );
      }
    }
  }
...
class CommandWidget {
  ...
  pkRetrievedListener(pk?: unknown) {
    // this.windowManager.showMain();
    // this.windowManager.send('balance', 'refresh');
    this.onPkRetrievalCb(this.pk);
  }

  setOnPkRetrieval(cb: (pk: unknown) => void) {
    this.onPkRetrievalCb = cb;
  }
...
export const init = (onPkRetrieval: (pk: unknown) => void) => {
  let mainWidget = initCommandWidget();
  mainWidget.setOnPkRetrieval(onPkRetrieval);
  mainWidget.initPK();
  // debugger;
};
